...
static void _rshift_word(size_t NumWords, BigInt_t *A, size_t nwords);
...
{
    memset(BigInt, 0, BigIntWordSize * NumWords);
}
...
uint32_t BigInt_to_int(size_t NumWords, BigInt_t *BigInt)
{
    uint32_t ret = 0;
...
uint64_t BigInt_to_long(size_t NumWords, BigInt_t *BigInt)
{
    uint64_t ret = 0;

/* Endianness issue if machine is not little-endian? */
#if (BigIntWordSize == 1)
    ret += (uint64_t)BigInt[0];
    ret += ((uint64_t)BigInt[1]) << 8;
    ret += ((uint64_t)BigInt[2]) << 16;
    ret += ((uint64_t)BigInt[3]) << 24;
    ret += ((uint64_t)BigInt[4]) << 32;
    ret += ((uint64_t)BigInt[5]) << 40;
    ret += ((uint64_t)BigInt[6]) << 48;
    ret += ((uint64_t)BigInt[7]) << 56;
#elif (BigIntWordSize == 2)
    ret |= (uint64_t)BigInt[0];
    ret |= ((uint64_t)BigInt[1]) << 16;
    ret |= ((uint64_t)BigInt[2]) << 32;
    ret |= ((uint64_t)BigInt[3]) << 48;
#elif (BigIntWordSize == 4)
    ret |= (uint64_t)BigInt[0];
    ret |= ((uint64_t)BigInt[1]) << 32;
#elif (BigIntWordSize == 8)
    ret += (uint64_t)BigInt[0];
#endif
...
{
    --NumWords;
    while (BigInt[NumWords] == 0 && NumWords > 0)
        --NumWords;
    return ++NumWords;
}
...
void BigInt_from_string(size_t NumWords, BigInt_t *BigInt, char *str)
{
    ...
    BigInt_t *temp = malloc(NumWords * BigIntWordSize);
    BigInt_t digit;
    BigInt_t ten = 10;
...
void BigInt_from_hex_string(size_t NumWords, BigInt_t *BigInt, char *Str)
{
    ...
    if (num_words * (BigIntWordSize * 2) < length)
        ++num_words; /* round up */
...
{
    BigInt_t *row = malloc(NumWords * BigIntWordSize);
    BigInt_t *tmp = malloc(NumWords * BigIntWordSize);
    size_t i, j;
...
static void BigInt_Karatsuba_internal(size_t num1_NumWords, BigInt_t *num1, size_t num2_NumWords, BigInt_t *num2, size_t Out_NumWords, BigInt_t *Out, int rlevel) /* Out should be XWords + YWords in size to always avoid overflow */
{
    ...
    if (num1_NumWords == 1 && num2_NumWords == 1)
    {
        ...
        if (Out_NumWords == 2)
        {
            BigInt_FROM_INT(Out, result);
        }
        else
            BigInt_from_int(Out_NumWords, Out, result);
        return;
    }
    ...
    BigInt_t *z0 = malloc(z0_NumWords * BigIntWordSize);
    size_t z1_NumWords = (MAX(low1_NumWords, high1_NumWords) + 1) + (MAX(low2_NumWords, high2_NumWords) + 1);
    BigInt_t *z1 = malloc(z1_NumWords * BigIntWordSize);
    ...
    if (use_out_as_z2)
    {
        BigInt_zero(Out_NumWords - (z2_NumWords), Out + z2_NumWords);
    } /* The remaining part of Out must be ZERO'D */
    BigInt_t *z2 = (use_out_as_z2) ? Out : malloc(z2_NumWords * BigIntWordSize);
    ...
    /* make z1 */
    {
        ...
        BigInt_t *low1high1 = malloc(low1high1_NumWords * BigIntWordSize);
        BigInt_t *low2high2 = malloc(low2high2_NumWords * BigIntWordSize);
    ...
    if (!use_out_as_z2)
        BigInt_copy_dif(Out_NumWords, Out, z2_NumWords, z2);
...
{
    BigInt_t *current = malloc(NumWords * BigIntWordSize);
    BigInt_t *denom = malloc(NumWords * BigIntWordSize);
    BigInt_t *tmp = malloc(NumWords * BigIntWordSize);
...
{
    BigInt_t *low = malloc(NumWords * BigIntWordSize);
    BigInt_t *high = malloc(NumWords * BigIntWordSize);
    BigInt_t *mid = malloc(NumWords * BigIntWordSize);
    BigInt_t *tmp = malloc(NumWords * BigIntWordSize);
...
{
    memcpy(Dst, Src, NumWords * BigIntWordSize);
}
